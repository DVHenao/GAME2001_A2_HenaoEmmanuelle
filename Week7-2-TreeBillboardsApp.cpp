...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	for (int i = 0; i < 1; ++i)
	{
		...
		XMStoreFloat4x4(&wallRitemFront->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 2.0f, -10.0f));
		wallRitemFront->ObjCBIndex = ObjCBIndex++;
		wallRitemFront->Mat = mMaterials["brickType1"].get();
		wallRitemFront->Geo = mGeometries["boxGeo"].get();
		...
		mRitemLayer[(int)RenderLayer::AlphaTested].push_back(wallRitemFront.get());

		XMStoreFloat4x4(&wallRitemBack->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 2.0f, 10.0f));
		...
		XMStoreFloat4x4(&wallRitemLeft->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 2.0f, 10.0f) * XMMatrixRotationAxis(yAxis, degreeRotation90));
		...
		XMStoreFloat4x4(&wallRitemRight->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 2.0f, -10.0f) * XMMatrixRotationAxis(yAxis, degreeRotation90));
